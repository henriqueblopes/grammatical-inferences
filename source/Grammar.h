...
public:
    ...
    Grammar(std::vector<Symbol> terminals, std::vector<Symbol> nonterminals, std::vector<Rule> rules,
            Symbol start, int nNonTerminals);
...
private:
    ...
    double *** CYKProb(const std::string& w);
    double **** CYKProbKL(const std::string& w);
    ...
    void pTiTiMinus1 (const std:: string& w);
